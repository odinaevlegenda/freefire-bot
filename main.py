...
TOKEN = os.environ.get("BOT_TOKEN", "8924908374:AAF6cctZO-gh35sBKu-uU0ntoRtjP38VLgE")
ADMIN_ID = int(os.environ.get("ADMIN_ID", 6895966276))
CHANNEL_ID = "@of5_bio"

bot = telebot.TeleBot(TOKEN)

# НАРХНОМАҲО (Ҳамаи номҳо якхела ва дуруст карда шуданд)
PRICES = {
...
def send_main_menu(chat_id):
    ...
    markup.add(
        types.InlineKeyboardButton(text="𝒅𝒊𝒂𝒎𝒐𝒏𝒅 𝒕𝒐 𝙵𝒓ее 𝙵𝚒𝒓е 💎", callback_data="buy_diamonds"),
        types.InlineKeyboardButton(text="𝒗𝒐𝒖𝒄𝒉𝒆𝒓 𝒕𝒐 𝙵𝒓ее 𝙵𝚒𝒓е 🎫", callback_data="buy_vouchers")
    )
    bot.send_message(chat_id, """Ассалому Алейкум 🤖🤝👤

    🤖• дар бораи бот •🤖

Ин бот барои алмос ( алмаз ё ки 𝒅𝒊𝒂𝒎𝒐𝒏𝒅 ) гузаронидан ба бозии 𝙵𝚛ее 𝙵𝚒𝒓е аст ‼️
...
def start(message):
    ...
    if check_sub(user_id):
        ...
        markup.add(types.InlineKeyboardButton(text=" O F 5 🔹", url="https://t.me/of5_bio"),
...
def ask_id(call):
    ...
    mode_text = {
        "buy_diamonds": "𝒅𝒊𝒂𝒎𝒐𝒏𝒅 𝒕𝒐 𝙵𝒓ее 𝙵𝚒𝒓е 💎",
...
def payment(call):
    chat_id = call.message.chat.id
    price = user_data.get(chat_id, {}).get('price', 0)
    markup = types.InlineKeyboardMarkup(row_width=1)
    if call.data == "pay_dc":
...
def send_admin_panel(message):
    if message.content_type == 'photo':
        cid = message.chat.id
        u_info = user_data.get(cid, {})
        boss_msg = f"👤 КОРБАР : {message.from_user.first_name}\n🛍 МАХСУЛОТ : {u_info.get('pack', 'N/A')}\n💷 МАБЛАГ : {u_info.get('price', 0)} 🇹🇯\n🆔️ БОЗИ : {u_info.get('game_id', 'N/A')}"
        markup = types.InlineKeyboardMarkup()
        ...
        bot.send_message(cid, "Расми чек қабул карда шуд ✅\n⏳.....интизори админ.....⏳")
...
def logic_btns(call):
    ...
    if call.data == "none":
        ...
        bot.edit_message_caption("Ҷавоб дода шуд ✅", chat_id, call.message.message_id)
